...
export const SettingsNav = memo(function SettingsNav({ active, onChange }: SettingsNavProps) {
    ...
    return (
        <aside className="w-52 shrink-0 px-4 pt-10 pb-6 flex flex-col gap-0.5 border-r border-base">
            <h1 className="text-[22px] font-bold text-pri mb-5 px-2">Settings</h1>
            {NAV_ITEMS.map((item) => (
                <button
                    key={item}
                    type="button"
                    ...
                    className={`flex items-center justify-between w-full text-left px-3 py-2 rounded-lg text-[13.5px] transition-colors cursor-pointer ${
                        active === item
                            ? "bg-tertiary text-pri font-medium"
                            : "text-sec hover:text-pri hover:bg-tertiary"
                    }`}
                >
                    <span>{item}</span>
